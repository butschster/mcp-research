...
/**
 * Converts [[E3]], [[R2:E5]], [[R2]], [[RM1]], [[RM1:N3]], [[SS2]] and [[T4]] references in text to clickable HTML links.
 *
...
export function linkRefs(text: string, researchSlug: string): string {
  ...
  return text.replace(/\[\[([^\]]+)\]\]/g, (_, ref: string) => {
    ...
    } else if (/^SS\d+$/.test(ref)) {
      // [[SS2]] — a session. A document that came out of an interview names it
      // this way; without the branch it fell through to /entry/SS2.
      if (shared && !include.sessions) return ref
      href = sessionPath(researchSlug, ref)
