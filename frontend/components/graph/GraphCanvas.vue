<template>
  <div class="graph-main">
    ...
    <!-- role="img": the canvas is a picture to assistive technology and not a
         tab stop. The keyboard path is the list view the page renders beside
         it; the label says so. -->
    <canvas
      v-show="!loading"
      ref="canvasRef"
      class="graph-canvas"
...
/**
 * The force-directed canvas: simulation, painting, pan/zoom, hover, focus mode.
 *
 * It owns nothing about where the data came from or where a node leads. The
 * owner's page and the shared page both render it; they differ in the fetch
 * and in the destinations, and those stay in the pages. Before this extraction
 * the second page would have been a 500-line copy of the first.
 */
...
const props = withDefaults(defineProps<{
  ...
  showArrows: boolean
  focusDepth: number
  /** The node in focus mode, or null. Right-click toggles it; the page may clear it. */
  focusedNodeId: string | null
  ariaLabel?: string
}>(), { ariaLabel: 'Knowledge graph' })

const emit = defineEmits<{
  /** Double-click on a node. The page decides whether it leads anywhere. */
  'open-node': [node: GraphNode]
  'update:focusedNodeId': [id: string | null]
  /** How many nodes and edges survived the filters. */
...
function updateFocusSets() {
  ...
  const depth = props.focusDepth
  const visited = new Set<string>([props.focusedNodeId])
  const edgeSet = new Set<string>()
  let frontier = [props.focusedNodeId]
...
function buildSimulation() {
  const filteredNodes = props.nodes.filter(n => props.visibleNodeTypes.has(n.type))
  let nodeIds = new Set(filteredNodes.map(n => n.id))

  let filteredEdges = props.edges.filter(e => {
    const srcId = typeof e.source === 'string' ? e.source : e.source.id
    const tgtId = typeof e.target === 'string' ? e.target : e.target.id
    return props.visibleEdgeTypes.has(e.type) && nodeIds.has(srcId) && nodeIds.has(tgtId)
  })

  // Determine connected nodes for orphan filtering
  let connectedNodeIds: Set<string> | null = null
  if (props.hideOrphans) {
  ...
  emit('counts', simNodes.length, simEdges.length)
...
/** Zooms and pans so every node is inside the canvas, with a margin. */
function fit() {
  const canvas = canvasRef.value
  ...
  // Labels hang off the right of a node, so the box gets room on that side.
  const pad = 60
...
function render() {
  ...
  const hasFocus = props.focusedNodeId !== null
  ...
  for (const node of simNodes) {
    ...
    const isFocused = node.id === props.focusedNodeId
...
function onContextMenu(e: MouseEvent) {
  ...
  const node = findNodeAt(wx, wy)
  if (node) {
    emit('update:focusedNodeId', props.focusedNodeId === node.id ? null : node.id)
  }
}
...
// Filters and data rebuild the simulation; the viewport is kept (see above).
watch(
  () => [props.visibleEdgeTypes, props.visibleNodeTypes, props.hideOrphans, props.nodes, props.edges],
  () => { if (!props.loading) buildSimulation() },
  { deep: true },
)

// The canvas is v-show'd, so while loading it has no size to measure. The
// first build waits for the frame in which it appears.
...
// Focus changes repaint without a rebuild: the simulation is untouched.
watch(() => [props.focusedNodeId, props.focusDepth], () => {
  updateFocusSets()
  render()
})

watch(() => props.showArrows, () => render())
