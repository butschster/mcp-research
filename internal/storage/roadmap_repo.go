...
func (r *RoadmapNodeRepository) Create(ctx context.Context, node *domain.RoadmapNode) error {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO roadmap_nodes (id, code, roadmap_id, title, description, node_type, status, position_x, position_y, parent_id, ref_type, ref_id, metadata, stage, node_date, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		refType, refID, metadata, node.Stage, node.NodeDate,
		now, now,
	)
	if err != nil {
...
func (r *RoadmapNodeRepository) Update(ctx context.Context, node *domain.RoadmapNode) error {
	...
	_, err := r.db.ExecContext(ctx,
		`UPDATE roadmap_nodes SET title=?, description=?, node_type=?, status=?, position_x=?, position_y=?, parent_id=?, ref_type=?, ref_id=?, metadata=?, stage=?, node_date=?, updated_at=? WHERE id=?`,
		node.Title, node.Description, node.NodeType, node.Status,
		node.PositionX, node.PositionY, parentID, refType, refID, metadata, node.Stage, node.NodeDate, now, node.ID,
	)
	if err != nil {
...
func (r *RoadmapNodeRepository) FindByID(ctx context.Context, id string) (*domain.RoadmapNode, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, code, roadmap_id, title, description, node_type, status, position_x, position_y, parent_id, ref_type, ref_id, metadata, stage, node_date, created_at, updated_at
...
func (r *RoadmapNodeRepository) scanNode(row *sql.Row) (*domain.RoadmapNode, error) {
	...
	err := row.Scan(
		...
		&refType, &refID, &metadata, &n.Stage, &n.NodeDate,
		&createdAt, &updatedAt,
	)
	if err == sql.ErrNoRows {
		return nil, nil
