...
func (r *SectionRepository) Create(ctx context.Context, section *domain.Section) error {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO sections (id, code, research_id, name, display_name, description, status, position, field_spec, spec_version, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		marshalJSON(section.FieldSpec), section.SpecVersion,
		now, now,
	)
	if err != nil {
...
func (r *SectionRepository) Update(ctx context.Context, section *domain.Section) error {
	now := time.Now().UTC().Format(time.DateTime)
	_, err := r.db.ExecContext(ctx,
		`UPDATE sections SET name=?, display_name=?, description=?, status=?, position=?, code=?, field_spec=?, spec_version=?, updated_at=?
		 WHERE id=?`,
		section.Name, section.DisplayName, section.Description,
		section.Status, section.Position, section.Code,
		marshalJSON(section.FieldSpec), section.SpecVersion,
		now, section.ID,
	)
	if err != nil {
...
func (r *SectionRepository) FindByID(ctx context.Context, id string) (*domain.Section, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, code, research_id, name, display_name, description, status, position, field_spec, spec_version, created_at, updated_at
